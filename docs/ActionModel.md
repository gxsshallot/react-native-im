...
将[标准消息](Conversation#消息)展示成UI组件。

默认部分：按照消息的类型来进行划分，注册处理方式时，可以用消息类型作为二级分区。

特殊处理判断：传递`message`为[标准消息](Conversation#消息)，返回布尔值表示是否匹配成功。
...
| 参数 | 类型 | 说明 |
| :-: | :-: | :- |
| message | [ImMessage](Conversation#消息) | 标准消息 |
...
解析操作，将原始消息，转换为[标准消息](Conversation#消息)。

特殊处理判断：传递`message`为原始消息，返回布尔值表示是否匹配成功。

处理方式：传递`message`为原始消息，返回`newMessage`为[标准消息](Conversation#消息)。

## Send

将[标准消息](Conversation#消息)发送到指定会话。
...
| 参数 | 类型 | 说明 |
| :-: | :-: | :- |
| imId | string | 消息的[会话ID](Conversation#会话对象) |
| chatType | number | 消息的[会话类型](Conversation#会话类型) |
| message | [ImMessage](Conversation#消息) | 标准消息 |
| ext | object | 扩展字段 |

## Abstract

将[标准消息](Conversation#消息)缩略成摘要信息，展示在会话的单元格中。
